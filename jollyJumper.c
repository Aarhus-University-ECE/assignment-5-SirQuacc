...
/*post: answer to exercise 7.a*/
    ...
    bool diffs_found[size]; // Bool array to keep track of what diff values have been found
    for(int i = 0; i < size; i++){ // Set all slots in array as false for starters.
        diffs_found[i] = false;
    }
    for(int i2 = 0; i2 < size-1; i2++){ // Go through the input array, and evaluate the absolute difference between the numbers.
